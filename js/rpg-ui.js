const RPGUI = {
    ...
    updateCharacterInfo() {
        ...
        if (document.getElementById('characterLevel')) {
            ...
            const baseDamage = info.attack * (1 + info.monsterDamage / 100);
            const critMultiplier = (info.criticalDamage - 100) / 100;
            const critChance = Math.min(info.criticalChance / 100, 1);
            const combatPower = Math.floor(baseDamage * (1 + critMultiplier * critChance));
            document.getElementById('characterCombatPower').textContent = combatPower;
            
            const survivability = Math.floor(info.maxHealth * (1 + info.dodge / 100) + info.defense * 10);
            document.getElementById('characterSurvivability').textContent = survivability;
