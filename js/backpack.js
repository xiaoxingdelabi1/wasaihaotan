const Backpack = {
    canAddItem(type, amount) {
        ...
        else if (type === 'dullPearl') newDullPearls += amount;
        ...
        if (newPurses > 0) {
            ...
            purseSlots = Math.ceil(newPurses / purseCap);
        }
        let piggyBankSlots = 0;
        if (newPiggyBanks > 0) {
            const piggyBankCap = Utils.getItemCapacity('小存钱罐');
            piggyBankSlots = Math.ceil(newPiggyBanks / piggyBankCap);
        }
        let dullPearlSlots = 0;
        if (newDullPearls > 0) {
            const dullPearlCap = Utils.getItemCapacity('暗淡的珍珠');
            dullPearlSlots = Math.ceil(newDullPearls / dullPearlCap);
        }
